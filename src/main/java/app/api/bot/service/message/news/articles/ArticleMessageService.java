...
public class ArticleMessageService {
  ...
  public void sendArticles(long chatId, List<ArticleDto> articles) {
  ...
  public void updateArticleAndArticleMenu(long chatId, int messageId, long articleId) {
    ...
    String text = article.getStatusOfWatchingBriefContent()
      ...
      : getMessageForArticleWithoutBrief(ArticleMapper.toDto(article));
    ...
    editMessage.setReplyMarkup(articleMenuInlineKeyboard.createArticleKeyboard(ArticleMapper.toDto(article)));
  ...
  private String getMessageForArticleWithoutBrief(ArticleDto article) {
    return "\uD83D\uDCCB " + article.getName() + "\n Категория: \"" + (article.getCategory() != null ? article.getCategory() : "Без категории")
